// acctcols_dump.mjs — bundles + runs `acctcols.dump.tsx`.
//
...
const out = path.join(HERE, '..', 'node_modules', '.orgtree-acctcols')
rmSync(out, { recursive: true, force: true })
mkdirSync(out, { recursive: true })

await esbuild.build({
  entryPoints: [path.join(HERE, 'acctcols.dump.tsx')],
  outdir: out,
  bundle: true,
  format: 'esm',
  platform: 'node',
...
if (!dest) {
  console.error('usage: node tests/acctcols_dump.mjs <out.html>')
  process.exit(2)
}
execFileSync(process.execPath, [path.join(out, 'acctcols.dump.mjs'), dest],
  { stdio: 'inherit' })
