...
test('desk header has bounded controls and a separate wrapping metadata row', async (t) => {
  ...
  assert.ok(top && meta)
  assert.equal(top.querySelector('.cc-info-cluster,.turn-status-banner'), null,
    'the nested status cluster can strand an Idle-only line at desk width')
...
test('fresh desk preserves empty context, off process, and turn-time slots', async (t) => {
  ...
  assert.equal(top.querySelector('.turn-status-banner'), null)
  assert.doesNotMatch(top.textContent ?? '', /Idle/)
})
