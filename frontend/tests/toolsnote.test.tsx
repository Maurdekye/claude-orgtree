...
import { setOpenRouterTiers, tierToolsNote, toolsNote } from '../src/canvas/shared'
...
test('the picker distinguishes all three states, and the favourite strip '
  ...
  assert.ok(titles.includes(SUPPORTED), `row titles: ${JSON.stringify(titles)}`)
  assert.ok(titles.includes(NOT_SUPPORTED), `row titles: ${JSON.stringify(titles)}`)
  assert.ok(titles.includes(UNKNOWN), `row titles: ${JSON.stringify(titles)}`)
