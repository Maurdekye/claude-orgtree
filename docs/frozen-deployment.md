...
| `providers` | per provider (`claude` required; `openai`/codex and `google`/antigravity optional) — for an npm-installed CLI its installed package version, npm `sha512-...` integrity, and that it was resolved from orgtree's own private prefix rather than bare `PATH`; for the Antigravity CLI (`kind: binary`, a native binary with its own installer) its version and the binary's own sha256, resolved from the installer's location | `PROVIDER_VERSION`, `PROVIDER_INTEGRITY`, `PROVIDER_SOURCE` |
