...
- Review corrections (agy-journal, 2026-09-05; no production change): two checks that could not fail. Both antigravity scenarios were walls, so `ceiling_kill_recorded` could sit permanently False and replay's `walled and` gate on `reset_known` could be dropped unnoticed; and no control showed the recomputed codex `kind` follows the evidence rather than `kind_recorded`. Added: (A) a real not-walled antigravity failure past a 1s (patched) ceiling through the fake CLI (`FAKEANTIGRAVITY_SCENARIO=plain_error`, a stall before init, an error sentence with no "limit" that names a reset) — not frozen, the ceiling raise ran, `reset_in_s` carried as evidence but not-walled / reset unknown / probe / ceiling kill recorded and recomputed, flipped decisions drift, the kill verdict follows the timing; (B) the real codex fixture with its machine tag re-keyed to `unauthorized` re-decides to auth and drifts against the recorded usage-limit. Suite 45/45; the three accepted mutants (capture hardcodes the ceiling verdict False; replay's reset_known ungated; replay derives the code from `kind_recorded`) are now each rejected by exactly the new check: evidence/mutants-failfix-m2b.log. Antigravity lane suites at the correction: dispatch 30/30, stream-order 112/112, windows PASS.
