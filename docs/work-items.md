...
| `owner` `{node, generation}` | **the assignment, and assignment IS ownership**: who is responsible, holds the item's management rights and receives the user's replies by default (a reply may explicitly address a participant), at the generation assigned; `owner_current` / `owner_state` say whether that seat is still live and unchanged |
...
| `participants` | node ids with narrow collaborator rights (below). `create(participants=...)` (including staffed creation) and later `participants add` send each **actually new member in the final membership** a passive participation notice (kind `notice`: read at its next turn, never a wake); duplicates, re-adds, self-adds, removals, an add-and-remove in one call, and the owner send nothing, and a member the actor may not address is reported in the result as `notice_refused` rather than refusing the membership change. Automatic participation notices preserve existing permissions: they do not grant the participant a reply audience to the sender, including for a deeper descendant; normal explicit mail keeps its existing audience behavior |
| `reply_recipients` | **derived on read, never stored**: who a user reply may be addressed to — `[{node, role: owner\|participant, state: live\|retired\|missing}]`, the owner first (when there is one), then the participants in stored order, the owner deduped out. `state` comes from the node table at read time on the same convention as `owner_state` (absent → missing; present but not live → retired), so a retired lineage predecessor the tree no longer shows is still listed. The panel renders this list; it is not trusted with it — the reply route re-derives the set at send time |
| `done_so_far`, `working_on_next` | **the docket status** — the latest two lists |
| `docket_at`, `last_updater` | when the latest docket update was written and by which agent. `last_updater` is HISTORY: it does not select the reply recipient (`owner` by default, or an explicitly selected participant) |
...
| `POST …/work-items/{slug}/reply {body, to?}` | user mail defaults to the **owner** — the assignment — exactly, prefixed `[DOCKET REPLY · slug "title"]` (the canonical name, not the caller's spelling); 422 when no recipient is specified and the item has no owner, or when the chosen node is gone, `deferred: true` when it is archived. There is **no fallback to the last updater**: never a substitute recipient. **`to`** (user 2026-09-06) addresses the reply to the owner or to one of the participants instead; it is validated against the **stored** owner + participants under the lock at send time — a name that is neither, or a member whose node no longer exists, is 422 with nothing sent, nothing saved and nobody substituted (a participant removed after the panel rendered is refused, not redirected to the owner). Ownership never moves. Response adds `role: owner\|participant`; a participant's mail and wake say it is addressed **as a participant** and name the owner |
