...
| `participants` | node ids with narrow collaborator rights (below). `create(participants=...)` (including staffed creation) and later `participants add` send each **actually new member in the final membership** a passive participation notice (kind `notice`: read at its next turn, never a wake); duplicates, re-adds, self-adds, removals, an add-and-remove in one call, and the owner send nothing, and a member the actor may not address is reported in the result as `notice_refused` rather than refusing the membership change |
