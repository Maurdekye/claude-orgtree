...
| `participants` | node ids with narrow collaborator rights (below). An agent-driven `participants add` sends each **actually new** member a passive participation notice (kind `notice`: read at its next turn, never a wake); re-adds, self-adds, removals and the owner send nothing, and a member the actor may not address is reported in the result as `notice_refused` rather than refusing the membership change |
...
| `POST …/work-items/{slug}/reply {body, to?}` | user mail to the **owner** — the assignment — exactly, prefixed `[DOCKET REPLY · slug "title"]` (the canonical name, not the caller's spelling); 422 when the item has no owner, 404 when that node is gone, `deferred: true` when it is archived. There is **no fallback to the last updater**: never a substitute recipient. **`to`** (user 2026-09-06) addresses the reply to the owner or to one of the participants instead; it is validated against the **stored** owner + participants under the lock at send time — a name that is neither, or a member whose node no longer exists, is 422 with nothing sent, nothing saved and nobody substituted (a participant removed after the panel rendered is refused, not redirected to the owner). Ownership never moves. Response adds `role: owner\|participant`; a participant's mail and wake say it is addressed **as a participant** and name the owner |
