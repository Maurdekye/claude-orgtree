...
- **It never copies**: `dispose` and `end` events are excluded from the derivation (mutant M4). A `partial` or `truncated` record, one with a HEAD/TAIL gap (`dropped` > 0 — the omitted middle can hide the deciding event, an unrecoverable owner or an interrupt, even with the tail kept), or one with no events, is **insufficient**: the summary says so (`evidence`, `gapped`), implies nothing about the outcome or phase, and reports no outcome drift (M5); the retained timeline is still printed and counted, and its ORDER is checked regardless — a gap can jump `seq` but never reverse it, so an inversion on a partial, truncated or gapped record is still `drift: ['order']` and `--assert` exits 1.
