...
| `api_retry` | `code` (the CLI's typed vocabulary or other), `n` | `system/api_retry` |
...
| `freeze` | `freeze_kind` (limit / connection), `run`, `delay_s`, `schedule` (observed-deadline / probe / backoff), `reset_known` | the freeze writes |
| `abandon` | `door` (pre_model / ran_then_failed / killed), `hard_fail_run` | `_turn_abandoned` / `_retry_exhausted` |
...
`outcome` is written by `Recorder.dispose`, called by the exit path that knows: **the last call before close wins**, and every call is also a `dispose` event so the record shows each claim in order. The dispositions and where they are set:
...
- **It never copies**: `dispose` and `end` events are excluded from the derivation (mutant M4). A `partial` or `truncated` record, or one with no events, is **insufficient**: the summary says so, implies nothing, and drifts against nothing (M5).
...
- `test_turn_events.py`: 36/36 on the branch. §1 schema/coercion/canaries/shapes; §2 recorder (six-thread order, head/tail drop with counts, stale emit and close refused, cost null vs 0.0, CAP truncation, ring at open and close over 75 leftovers, off switch, the under-the-lock probe); §3 summarize/drift (frozen, never-copied, precedence, five phases plus a codex admission, insufficient, fixture names and containment); §4 claude lane through the fake CLI — completed, died-in-flight (fixture resolves and agrees), 401 with planted secrets (abandoned), dead-on-arrival (pre_model), hang with `TURN_IDLE`=1 (killed), manual interrupt; §5 OpenRouter typed 429 (frozen); §6 codex tool / usage_limit / plain_failure; §7 antigravity text / usage_limit / plain_error past a 1 s ceiling / a real mid-turn ⏸; §8 fail-open ×3 against the control plus a positive control; §9 the inspector under the hook (renders, drifts on an edited outcome, reports an unresolved fixture, renders a stub as partial, hook control).
- **Red proof** on main 8d63b74's supervisor with the new modules copied in (evidence/turnlog-red-8d63b74.log): exactly the 14 real-lane checks fail (no record is written), §8/§9 declare themselves inert, the 15 pure checks pass.
- **Mutants** for the 15 pure checks (evidence/mutants-turnlog.log): M1 stamp outside the lock (rejected only after the under-the-lock probe was added — the ordering assertions alone let it survive), M2 dropped not counted, M3 unknown string kept, M4 summary copies end, M5 partial not insufficient, M6 basename validation, M7 no eviction at open, M8 watchdog overrides a later freeze, M9 over-cap unflagged, M10 unknown cost 0.0, M11 stale emit accepted, M12 handled terminal overrides the freeze, M13 admission from any typed status, M15 lists uncapped — all rejected. **M14** (containment check removed) survives and is an equivalent mutant: the strict name regex admits no separator, so no name reaching `fixture_path` can leave the directory; the check is defence in depth and is kept, not credited.
...
- Sibling suites on the branch: failure-fixtures 45/45, provider-limit-freeze 12/12, codex-dispatch 25/25, antigravity-dispatch 30/30, route-fallback-scope 25/25, limit-freeze 268/268; turn-lifecycle and luna-reserve-route: see the review packet.
